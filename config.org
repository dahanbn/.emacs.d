...
#+BEGIN_SRC emacs-lisp
  ...
  (if (or (string= (getenv "USERNAME") "HannaskeD")
...
#+begin_src emacs-lisp
  (use-package slime
    ...
    (when (string-equal system-type "windows-nt")
      ;; (setq slime-lisp-implementations '((sbcl ("c:/Program Files/Steel Bank Common Lisp/1.4.2/sbcl.exe" "--core" "c:/Program Files/Steel Bank Common Lisp/1.4.2/sbcl.core"))))
    ...
    (if (file-exists-p (expand-file-name (quicklisp/slime-helper.el) dh/user-dir))
        (load (expand-filename (quicklisp/slime-helper.el) dh/user-dir))
...
#+begin_src emacs-lisp
  (use-package geiser
    :if (or (executable-find "racket") (executable-find "guile")))
#+end_src
* hydra
...
#+begin_src emacs-lisp
  (when (and (boundp 'dh/dropbox-dir) (boundp 'dh/not-at-work))
    ...
    (setq org-default-notes-file (expand-file-name "0-dh-tasks.org" org-directory))
...
#+begin_src emacs-lisp
  ...
  (when (boundp 'dh/dropbox-dir)
    ...
    (push `("dropbox: org directory" . ,(expand-file-name "org" dh/dropbox-dir)) yal/launchable-items-emacs)

    ;; also add my most used org files for task management to yal
    (push `("tasks: org directory" . ,(expand-file-name org-directory)) yal/launchable-items-emacs)
    (push `("tasks: 0-dh-tasks.org" . ,(expand-file-name "0-dh-tasks.org" org-directory)) yal/launchable-items-emacs)
    (push `("tasks: refile-beorg.org" . ,(expand-file-name "refile-beorg.org" org-directory)) yal/launchable-items-emacs)
    (push `("tasks: refile-drafts.org" . ,(expand-file-name "refile-drafts.org" org-directory)) yal/launchable-items-emacs)
    (push `("tasks: notes.org" . ,(expand-file-name "notes.org" org-directory)) yal/launchable-items-emacs)
    (push `("tasks: misc.org" . ,(expand-file-name "misc.org" org-directory)) yal/launchable-items-emacs)
  ...
  (when (and (boundp 'dh/dropbox-dir) (boundp 'dh/dropbox-full-available))
    ...
    ;; (push `("ebook: " . ,(expand-file-name "dahan_ebooks/Verlage/.pdf" dh/dropbox-dir)) yal/launchable-items-system)
