...
** DISABLED Emacs: Making a hydra cheatsheet for Lispy
   ...
   Fortunately, the function reference had a link to [[https://raw.githubusercontent.com/abo-abo/lispy/gh-pages/index.org][the Org file used to
   generate it]]. I copied the tables, merged them together, named them
...
(eval
 (append
  '(defhydra dh/lispy-cheat-sheet (:hint nil :foreign-keys run)
     ("<f14>" nil :exit t))
  ...
  (when (boundp 'dh/me-at-work)
    ...
    (setq org-todo-keywords
          '((sequence "NEW(n!)" "IN_PROCESS(p!)" "WAITING_INT(i@/!)" "WAITING_EXT(e@/!)" "|" "DONE(d@)" "DELEGATED(@)" "CANCELLED(c@)")
            (sequence "O_NEW(N!)" "O_IN_PROCESS(P!)" "O_IN_BIDDING(B!)" "O_IN_FORMAL_BIDDING(F!)" "O_WAITING_INT(I@/!)" "O_WAITING_EXT(E@/!)" "O_APPROVAL(A!)" "O_Z2_APPROVAL(Z!)" "O_ORDERED(O@/!)" "O_DELIVERED(D@/!)" "|" "O_DONE(@)" "O_CANCELLED(@)")
    ...
    (setq org-todo-keyword-faces
          '(
            ...
            ("O_Z2_APPROVAL" . (:foreground "#C2A1FF" :weight bold)) ; monokai-violet-light
...
*** DISABLED Fixing YASnippets tab in org 
