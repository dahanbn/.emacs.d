...
#+begin_src emacs-lisp
  (when (boundp 'dh/not-at-work)
    ...
    (if (file-directory-p (expand-file-name "iCloudDrive" dh/user-dir))
...
** evil
make a slow try with evil:
 main goal is to have Vim like movements and actions as d2w in normal mode
#+BEGIN_SRC emacs-lisp
(use-package evil
  :ensure t
  :bind ()
...
#+begin_src emacs-lisp
  ...
  (when (boundp 'dh/iclouddrive-dir)
    ...
    (push `("iCloud: Textastic directory" .  ,(expand-file-name "M6HJR9W95L~com~textasticapp~textastic" dh/iclouddrive-dir)) yal/launchable-items-emacs))
  ...
  (when (boundp 'dh/dropbox-dir)
    ...
    (push `("tasks: refile-ifttt.org" . ,(expand-file-name "refile-ifttt.org" org-directory)) yal/launchable-items-emacs)
